% !TEX TS-program = xelatex
% !TEX encoding = UTF-8 Unicode
% -*- coding: UTF-8; -*-
% vim: set fenc=utf-8

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% cv.tex                                 %%
%% <https://github.com/aliyazdi75/Resume> %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\documentclass[letterpaper,11pt]{article}
\input{resume.tex}

%%%%%%%%%%%%%%%
%% VARIABLES %%
%%%%%%%%%%%%%%%

% CV Info.
\newcommand{\CVAuthor}{Ali Yazdi}
\newcommand{\CVTitle}{Ali Yazdi's Resume}
\newcommand{\TitleDesciption}{Senior Software Engineer, GDG \& Flutter Montreal Organizer}
\newcommand{\CVNote}{Last Update on \today}
\newcommand{\CVWebpageName}{yazdi.dev}
\newcommand{\CVWebpage}{https://yazdi.dev}
\newcommand{\MainEmailName}{ma.yazdi75@gmail.com}
\newcommand{\MainEmail}{mailto:ma.yazdi75@gmail.com}
\newcommand{\LinkedinName}{linkedin:aliyazdi75}
\newcommand{\Linkedin}{https://linkedin.com/in/aliyazdi75}
\newcommand{\GithubName}{Github:aliyazdi75}
\newcommand{\Github}{https://github.com/aliyazdi75}
\newcommand{\SecondoryEmailName}{a.yazdi@gdgmontreal.com}
\newcommand{\SecondoryEmail}{mailto:a.yazdi@gdgmontreal.com}
\newcommand{\PhoneName}{(438)\,929-3675}
\newcommand{\Phone}{tel:+14389293675}
\newcommand{\Address}{Montréal, Québec, Canada}
\newcommand{\PDFKeywords}{resume, cv, aliyazdi75, \CVAuthor, flutter, developer}

% PDF settings and properties.
\hypersetup{
pdftitle={\CVTitle},
pdfauthor={\CVAuthor},
pdfsubject={\CVWebpage},
pdfcreator={aliyazdi75},
pdfproducer={aliyazdi75},
pdfkeywords={\PDFKeywords},
}

% Adjust margins
\addtolength{\oddsidemargin}{-0.5in}
\addtolength{\evensidemargin}{-0.5in}
\addtolength{\textwidth}{1in}
\addtolength{\topmargin}{-.5in}
\addtolength{\textheight}{1.0in}

\raggedbottom
\raggedright
\setlength{\tabcolsep}{0in}

% Sections formatting
\titleformat{\section}{
  \vspace{-4pt}\scshape\raggedright\large
}{}{0em}{}[\color{black}\titlerule \vspace{-5pt}]

%%%%%%%%%%%%%%%%%%%%%
%% ACTUAL DOCUMENT %%
%%%%%%%%%%%%%%%%%%%%%

\begin{document}

%%%%%%%%%%%%%%%%%
%% TITLE BLOCK %%
%%%%%%%%%%%%%%%%%

\begin{center}
    \textbf{\Huge \CVAuthor} \\
    \vspace{1mm}
    \TitleDesciption \\
    \small {\txtcolor\href{\Phone}{\textbf{\PhoneName}}}
    $\vert$
    {\txtcolor\href{\MainEmail}{\textbf{\MainEmailName}}}
    $\vert$
    {\txtcolor\href{\Github}{\textbf{\GithubName}}}
    $\vert$
    {\txtcolor\href{\Linkedin}{\textbf{\LinkedinName}}}
    $\vert$
    {\txtcolor\href{\CVWebpage}{\textbf{\CVWebpageName}}}
\end{center}

%%%%%%%%%%%%%%%
%% OBJECTIVE %%
%%%%%%%%%%%%%%%

\Section{Objective}

Dedicated software engineer with a robust background in mobile, extensive experience
in backend and scalable cloud infrastructure, eager for a role prioritizing innovation and growth in a dynamic development environment.

%%%%%%%%%%%%%%%%
%% EXPERIENCE %%
%%%%%%%%%%%%%%%%

\Section{Experience}

\href{https://alagarderie.ca}
{\textbf{Senior Software Engineer}}, ÀlaGarderie (Contract), Montréal - Node.js, AWS, Flutter
\hfill \DatestampYM{2023}{11}\,--\,Present
\begin{itemize}[leftmargin=*, noitemsep, topsep=0pt, partopsep=0pt]
    \item Developed a secure and scalable AWS infrastructure using Terraform and maintained a robust Node.js backend and cross-platform Flutter web and mobile applications for fully automated dev and production environments.
\end{itemize}

\BigGap
\href{https://woopen.com}
{\textbf{Senior Flutter Developer}}, Woopen, Montréal - Flutter, Firebase, Node.js
\hfill \DatestampYM{2021}{08}\,--\,\DatestampYM{2023}{07}
\begin{itemize}[leftmargin=*, noitemsep, topsep=0pt, partopsep=0pt]
    \item Led Flutter development for a real estate app, enhancing performance and reducing bugs by 70\% through upgrades, code optimization, and migrating to null-safety and localization. Streamlined workflows by integrating automated tools and collaborating with agile teams for feature development.
\end{itemize}

\BigGap
\href{https://en.iranian.ac.ir}
{\textbf{Software Engineer}}, Iranian University, Tehran - Jira, Asp.net
\hfill \DatestampYM{2020}{04}\,--\,\DatestampYM{2021}{08}
\begin{itemize}[leftmargin=*, noitemsep, topsep=0pt, partopsep=0pt]
    \item Streamlined E-Learning registration and exams systems, developed APIs to boost server response times.
\end{itemize}

\BigGap
\href{https://tarafdari.com}
{\textbf{Mobile Developer}}, Tarafdari, Tehran - Flutter, Android, Laravel
\hfill \DatestampYM{2019}{01}\,--\,\DatestampYM{2020}{04}
\begin{itemize}[leftmargin=*, noitemsep, topsep=0pt, partopsep=0pt]
    \item Migrated a leading sports app to Flutter, adding live scores, streaming, and chat, boosting user engagements.
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PROFESSIONAL PROJECTS %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%

\Section{Professional Projects}

\href{https://toituresblp.ca}
{\textbf{Report System Application}}, Les Toitures BLP, Canada - Flutter, Firebase
\hfill \DatestampYM{2024}{01}\,--\,Present
\begin{itemize}[leftmargin=*, noitemsep, topsep=0pt, partopsep=0pt]
    \item Developed a report system application or professionals with Firebase and Flutter.
\end{itemize}

\BigGap
\href{https://github.com/aliyazdi75/flutter_chat}
{\textbf{Chat Application}}, Mate Machine, Canada - Flutter
\hfill \DatestampYM{2020}{07}\,--\,\DatestampYM{2021}{06}
\begin{itemize}[leftmargin=*, noitemsep, topsep=0pt, partopsep=0pt]
    \item Made a robust real-time P2P WebRTC chat and video call.
\end{itemize}

\BigGap
\href{https://gallery.yazdi.dev}
{\textbf{Alumni Gallery}}, Amirkabir University, Iran - Flutter
\hfill \DatestampYM{2021}{01}\,--\,\DatestampYM{2021}{02}
\begin{itemize}[leftmargin=*, noitemsep, topsep=0pt, partopsep=0pt]
    \item Developed a responsive Flutter gallery web app with a microservice architecture and advanced navigation.
\end{itemize}

\BigGap
\href{https://raw.githubusercontent.com/aliyazdi75/Resume/main/BScThesis-Abstract.pdf?token=GHSAT0AAAAAACNTPCQKF3DMPOJ3YIMG3IPUZO5MTGQ}
{\textbf{Location-Based Recommendation}}, Amirkabir University, Iran - Django, Flutter
\hfill \DatestampYM{2020}{07}\,--\,\DatestampYM{2020}{10}
\begin{itemize}[leftmargin=*, noitemsep, topsep=0pt, partopsep=0pt]
    \item Developed a recommendation system integrating content-based and collaborative filtering.
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%
%% VOLUNTEER EXPERIENCE %%
%%%%%%%%%%%%%%%%%%%%%%%%%%

\Section{Volunteer Experience}

\href{https://gdg.community.dev/gdg-montreal}
{\textbf{Organizer and Speaker}}, Google Developer Group, Montréal
\hfill \DatestampYM{2023}{03}\,--\,Present
\begin{itemize}[leftmargin=*, noitemsep, topsep=0pt, partopsep=0pt]
    \item Organized Flutter Montreal events, enhancing community engagement through meetups and workshops.
\end{itemize}

%%%%%%%%%%%%%%%
%% EDUCATION %%
%%%%%%%%%%%%%%%

\Section{Education}

\href{https://aut.ac.ir/en}
{\textbf{Bachelor in Computer Engineering}}, Amirkabir University of Technology, Iran
\hfill \DatestampYM{2015}{09}\,--\,\DatestampYM{2020}{09}
\begin{itemize}[leftmargin=*, noitemsep, topsep=0pt, partopsep=0pt]
    \item Thesis: Designing and Implementing a Location-Based Social Network Recommendation System.
\end{itemize}

%%%%%%%%%%%%
%% SKILLS %%
%%%%%%%%%%%%

\Section{Skills}

\textbf{Programming Languages}, Dart, Python, JavaScript, Java, HTML, CSS, C/C++, PHP, Assembly

\Gap
\textbf{Technologies}, Flutter, Firebase, AWS, Node.js, Django, GCP, Android, NoSQL, PostgreSQL, MySQL

\Gap
\textbf{Tools}, CI/CD, Docker, Git, Terraform, \LaTeX, WebRTC, Scikit-learn, Pandas, Bash Scripting

\Gap
\textbf{Other}, Jira, Sentry, Software Systems Engineering, Cross-Platform Application Development

%%%%%%%%%%%%%%%%
%% REFERENCES %%
%%%%%%%%%%%%%%%%

\Section{References}

\href{https://linkedin.com/in/leehack}
{\textbf{Jhinseok Lee}}, Co-organizer at GDG Montreal, \href{mailto:leehack@gmail.com}{leehack@gmail.com}

\Gap
\href{https://linkedin.com/in/tayormi}
{\textbf{Ajiboye Temitope}}, Colleague at Woopen, \href{mailto:mrtayormi@outlook.comm}{mrtayormi@outlook.com}


%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PROFESSIONAL ARTICLES %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%

\Section{Professional Articles}

\href{https://medium.com/flutter-community/lets-make-the-flutter-navigator-2-bc5953251c3e?source=friends_link&sk=e1ff30235ab93276b558c5b5ef1dd15f}
{\textbf{Flutter Navigator}}, Implementation of navigator 2 for flutter focuses on web apps.

\Gap
\href{https://medium.com/flutter-community/the-software-engineering-approach-in-cross-platform-programming-with-flutter-part-1-efcdc8a8fc26?source=friends_link&sk=ceb60f56685ce66c7f9574595748f25b}
{\textbf{Software Engineering Principles}}, A series detailing software engineering methods for cross-platform apps.

%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PROFESSIONAL PROJECTS %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%

\Section{Academic Projects}

\href{https://github.com/aliyazdi75/Planet_vs_Zombies}
{\textbf{Planet vs Zombies Game}}, Developed a 2D game using Java and object-oriented programming concepts.

\Gap
\href{https://github.com/aliyazdi75/NUMEX-Interpreter}
{\textbf{NUMEX Interpreter}}, Designed an interpreter using Racket as part of the Programming Languages course.

\Gap
\href{https://github.com/aliyazdi75/Finite-State-Machine}
{\textbf{Finite State Machine}}, Implemented AVL, TST, trie, and hash data structures for an efficient data search.

%%%%%%%%%%%%%%%
%% LANGUAGES %%
%%%%%%%%%%%%%%%

\Section{Languages}

\textbf{English}, Full

\Gap
\textbf{French}, Limited

\Gap
\textbf{Persian}, Native

\end{document}
